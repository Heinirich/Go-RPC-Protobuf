...
const (
	...
	StudentService_GetStudentsByName_FullMethodName = "/StudentService/GetStudentsByName"
)
...
//
// Service definition for student
type StudentServiceClient interface {
	GetStudentByID(ctx context.Context, in *SearchByID, opts ...grpc.CallOption) (*Student, error)
	GetStudentsByName(ctx context.Context, in *SearchByName, opts ...grpc.CallOption) (grpc.ServerStreamingClient[Student], error)
}
...
func (c *studentServiceClient) GetStudentsByName(ctx context.Context, in *SearchByName, opts ...grpc.CallOption) (grpc.ServerStreamingClient[Student], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &StudentService_ServiceDesc.Streams[0], StudentService_GetStudentsByName_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[SearchByName, Student]{ClientStream: stream}
...
type StudentService_GetStudentsByNameClient = grpc.ServerStreamingClient[Student]
...
type StudentServiceServer interface {
	...
	GetStudentsByName(*SearchByName, grpc.ServerStreamingServer[Student]) error
...
func (UnimplementedStudentServiceServer) GetStudentsByName(*SearchByName, grpc.ServerStreamingServer[Student]) error {
...
func _StudentService_GetStudentsByName_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(StudentServiceServer).GetStudentsByName(m, &grpc.GenericServerStream[SearchByName, Student]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type StudentService_GetStudentsByNameServer = grpc.ServerStreamingServer[Student]
